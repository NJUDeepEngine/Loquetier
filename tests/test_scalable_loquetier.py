...
def parse_args():
    ...
    parser.add_argument('-x', '--timestamp_txt', type=str,
                        help='timestamp txt file')
...
if __name__ == "__main__":
    ...
    timestamp_file = args.timestamp_txt
    ...
    with open(timestamp_file, 'r') as f:
        timestamps = [int(t) for t in f.readlines()]
    ...
    all_requests = {}
    mark_id = 1
    for testcase_id, testcase in enumerate(infer_config['testcases']):
        ...
        intervals = timestamps[testcase_id::len(infer_config['testcases'])]
        sample_num = len(intervals)
    ...
    gen_res, ft_res, time_cost, slo_info = run_scalable(
        model_gen,
        tokenizer_llama,
        trainers,
        input_list,
